use crate::helpers::{
    convert_between_linux_and_windows, is_windows_path, retrieve_directory_content,
};
                    ...
                    if hash_map.contains_key(&target_dir) {
                        ...
                        target_teleport_dir.par_iter().for_each(|file| {
                            ...
                            if file.is_dir() {
                                ...
                                dir::move_dir(
                                    &convert_between_linux_and_windows(&file),
                                    dest,
                                    &options,
                                )
                                .unwrap_or_else(|_| panic!("should transfer folders to {}", &dest));
                            }
                            if file.is_file() {
                                let destination =
